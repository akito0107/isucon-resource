...
func validateUser(accountName, password string) bool {
        for _, ss := range accountName{
            if !(ss>='a' && ss<='z' || ss>='A' && ss <= 'Z' || ss >= '0' && ss <= '9'){
		return false
		}
	}
	for _, ss :=  range password{
            if !(ss>='a' && ss<='z' || ss>='A' && ss <= 'Z' || ss >= '0' && ss <= '9'){
                return false
                }
	}
        if !(len(accountName)>2 && len(password)>5 ) {
		return false
	}
...
//func getUser(r *http.Request, UserID int) (User, error) {
func getUser(r *http.Request, UserID int) (User, error) {
	memcacheClient := memcache.New("localhost:11211")
	// GET FROM MEMCACHED
	memu, _ := memcacheClient.Get("user_" + strconv.Itoa(UserID))
	if memu != nil {
		u := User{}
		if err := json.Unmarshal(memu.Value, &u); err != nil {
			return User{}, err
		}
		return u, nil
	}
	...
	memcacheClient.Set(&memcache.Item{Key:"user_" + strconv.Itoa(UserID), Value:user_json})
	return u, nil
}
...
//			uidCast = uid.(int)
//		case int64:
//			uidCast = int(uid.(int64))
//	}

//	u, err := getUser(r, uidCast)
	if err != nil {
		return User{}
	}
	return u
...
func makePosts(r *http.Request, results []Post, CSRFToken string, allComments bool) ([]Post, error) {
	var posts []Post
...
//			cu, uerr := getUser(r, comments[i].UserID)
			if uerr != nil {
				return nil, uerr
			}
//			comments[i].User = cu
...
func getIndex(w http.ResponseWriter, r *http.Request) {
	...
	//err := db.Select(&results, "SELECT `id`, `user_id`, `body`, `mime`, `created_at` FROM `posts` LIMIT 40")
	err := db.Select(&results, "SELECT `id`, `user_id`, `body`, `mime`, `created_at` FROM `posts` LIMIT 40")
	if err != nil {
		fmt.Println(err)
		return
	...
	posts, merr := makePosts(r, results, getCSRFToken(r), false)
	if merr != nil {
		fmt.Println(merr)
		return
	}
...
func getAccountName(c web.C, w http.ResponseWriter, r *http.Request) {
	...
	rerr := db.Select(&results, "SELECT `id`, `user_id`, `body`, `mime`, `created_at` FROM `posts` WHERE `user_id` = ?  LIMIT 40", user.ID)
	if rerr != nil {
		fmt.Println(rerr)
		return
	}
	...
	commentCount := 0
	cerr := db.Get(&commentCount, "SELECT COUNT(id) AS count FROM `comments` WHERE `user_id` = ?", user.ID)
	if cerr != nil {
		fmt.Println(cerr)
		return
	}
	...
	if postCount > 0 {
		...
		ccerr := db.Get(&commentedCount, "SELECT COUNT(id) AS count FROM `comments` WHERE `post_id` IN ("+placeholder+")", args...)
		if ccerr != nil {
			fmt.Println(ccerr)
			return
		}
...
func getPosts(w http.ResponseWriter, r *http.Request) {
	...
	results := []Post{}
	rerr := db.Select(&results, "SELECT `id`, `user_id`, `body`, `mime`, `created_at` FROM `posts` WHERE `created_at` <= ?  LIMIT 40", t.Format(ISO8601_FORMAT))
	if rerr != nil {
		fmt.Println(rerr)
		return
	}
...
func getPostsID(c web.C, w http.ResponseWriter, r *http.Request) {
	...
	results := []Post{}
	rerr := db.Select(&results, "SELECT `id`, `user_id`, `body`, `mime`, `created_at` FROM `posts` WHERE `id` = ? LIMIT 40", pid)
	if rerr != nil {
		fmt.Println(rerr)
		return
	}

	posts, merr := makePosts(r, results, getCSRFToken(r), true)
	if merr != nil {
		fmt.Println(merr)
		return
	}
...
func postIndex(w http.ResponseWriter, r *http.Request) {
	...
	e := ioutil.WriteFile("/var/assets/image/"+strconv.FormatInt(pid, 10) + "."+ext, filedata, 0644)
	if e != nil {
...
func getImage(c web.C, w http.ResponseWriter, r *http.Request) {
	...
	post := Post{}
	derr := db.Get(&post, "SELECT `id`, `user_id`, `body`, `mime`, `created_at` FROM `posts` WHERE `id` = ?", pid)
	if derr != nil {
	...
	if ext == "jpg" && post.Mime == "image/jpeg" ||
		...
		//e := ioutil.WriteFile("/var/assets/image/"+strconv.Itoa(pid) + "."+ext, post.Imgdata, 0644)
		//if e != nil {
		//	log.Printf("%s", e.Error())
		//}
		if err != nil {
